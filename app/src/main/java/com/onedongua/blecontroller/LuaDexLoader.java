...
public class LuaDexLoader {
    ...
    public LuaDexLoader(String filesDir, Context context) {
    ...
    public LuaDexClassLoader loadApp(String pkg) {
        try {
            ...
            if (dex == null) {
                ...
                dex = new LuaDexClassLoader(info.publicSourceDir, LuaApplication.getInstance().getOdexDir(), info.nativeLibraryDir, context.getClassLoader());
    ...
    public void loadLibs() throws LuaException {
        File[] libs = new File(filesDir + "/libs").listFiles();
        if (libs == null)
            return;
        for (File f : libs) {
            if (f.isDirectory())
    ...
    public void loadLib(String name) throws LuaException {
        ...
        if (fn.startsWith("lib"))
            fn = fn.substring(3);
        String libDir = context.getDir(fn, Context.MODE_PRIVATE).getAbsolutePath();
        String libPath = libDir + "/lib" + fn + ".so";
        File f = new File(libPath);
        if (!f.exists()) {
            f = new File(filesDir + "/libs/lib" + fn + ".so");
            if (!f.exists())
                throw new LuaException("can not find lib " + name);
            LuaUtil.copyFile(filesDir + "/libs/lib" + fn + ".so", libPath);
    ...
    public DexClassLoader loadDex(String path) throws LuaException {
        ...
        if (dex == null) {
            ...
            if (dex == null) {
                dex = new LuaDexClassLoader(path, odexDir, LuaApplication.getInstance().getApplicationInfo().nativeLibraryDir, context.getClassLoader());
                dexCache.put(id, dex);
            }
        }
